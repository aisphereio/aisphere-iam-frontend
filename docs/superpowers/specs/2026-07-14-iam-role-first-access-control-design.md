...
1. 扩展 `.zed` 和权限清单，增加动态角色定义和测试。
2. 扩展 IAM 数据表、Proto、业务服务和投影逻辑。
3. 保留现有内置模板及其 Relation，不迁移已有内置 Grant。
4. 将已有非内置模板识别为 legacy alias；根据其 Relation 推导等价能力，管理员确认后转换为动态角色。
5. 发布新前端，合并导航和权限页面。
6. 观察稳定后删除旧注册表单、旧 `GrantsTab` 和重复页面。
7. 后续在 Skill 等业务前端按独立任务接入同一资源分享契约。
